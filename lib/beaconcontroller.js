...
class BeaconController extends Events
{
    constructor(log, options)
    {
        ...
        setInterval(this.onRestartScanHeartbeat.bind(this), this.options.restartScanHeartbeatInterval);
    ...
    onDiscovery(peripheral) {
        ...
        {
            peripheral.updateRssi(this.onUpdateRSSI.bind(this));
        }*/
    ...
    onUpdateRSSI(error, rssi)
    {
        this.log("RSSI Updated: " + rssi);
    }
    ...
    enableScanning()
    {
        ...
        {
            this.log("Failed to start scanning: " + error);
        }
    }
    ...
    onStateChange(state)
    {
        this.log("Processing state change. New start: " + state);
        if (state === 'poweredOn')
        {  
            this.enableScanning();
        }
        ...
        {
            this.disableScanning();
            // Force scanning to restart
            //setTimeout(this.enableScanning.bind(this), 3000);
        }
    }    
};
